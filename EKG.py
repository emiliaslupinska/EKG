...
# Ścieżka dostosowana
path_inp = r"D:\Nowy_folder\Ekg spoczynkowe .txt"
...
st.markdown(f"""
    <div class="moja-ramka">
        ...
        <p style="color: {jasny_fiolet};">laboratorium fizyki medycznej</p>
    </div>
    """, unsafe_allow_html=True)   
...
with col2:
    ...
    fig_pie = px.pie(
        ...
        hole=0.4,  
        color_discrete_sequence=[rozowy, jasny_fiolet],
...
with col_hrv:
    ...
    with c_left:
        ...
        fig_zoom = go.Figure()
        fig_zoom.add_trace(go.Scatter(x=df_zoom['czas'], y=df_zoom['ecg_filtered'], line=dict(color=rozowy, width=2)))
        fig_zoom.add_trace(go.Scatter(x=df_zoom['czas'].iloc[peaks_zoom], y=df_zoom['ecg_filtered'].iloc[peaks_zoom], mode='markers', marker=dict(color=fioletowy, size=10)))
    ...
    with c_right:
        ...
        if len(rr_intervals) > 0:
            ...
            fig_rr.add_trace(go.Scatter(
                ...
                y=rr_intervals, 
                mode='lines+markers',
                line=dict(color=fioletowy, width=1),
                marker=dict(size=4)
            ))
...
for i, name in enumerate(names):
        ...
        fig_emd.update_yaxes(
            range=[-y_max, y_max], 
            row=curr_row, col=1,
            showline=True, linewidth=1.5, linecolor=czarny,
            gridcolor='lightgrey'
        )
        ...
        fig_emd.add_annotation(
            dict(
                ...
                textangle=0,
                xanchor='right',
                font=dict(size=12, color=fioletowy)
            )
        )
...
fig_emd.update_xaxes(
        showline=True, 
        linewidth=1.5, 
        linecolor=czarny,  # Kolor linii osi
        title_text="<b>Time (samples)</b>", 
        title_font=dict(color=fioletowy, size=14), # Kolor napisu "Time (samples)"
        tickfont=dict(color=ciemny_fiolet), # Kolor numerków (0, 5k, 10k...)
        row=rows_count, 
        col=1
    )
...
if n_imfs >= 2:
        ...
        fig_clean.add_trace(go.Scatter(
            ...
            line=dict(color=fioletowy, width=1.5)
        ))
...
st.markdown(f"""<p style="font-size: 18px; font-weight: bold; color: {fioletowy};">Analiza Widmowa (FFT) - Przed i Po</p>""", unsafe_allow_html=True)
st.markdown(f"""<hr style="margin-top: -10px; height:5px; border:none; background-color:{fioletowy};" />""", unsafe_allow_html=True)
...
fig_fft.add_trace(go.Scatter(
        ...
        line=dict(color=ciemny_fiolet, width=2)
    ))

fig_fft.add_trace(go.Scatter(
        x=f_plot, y=fft_clean_plot, 
        name="Widmo po usunięciu oddechu", 
        line=dict(color=rozowy, width=1.5)
    ))
